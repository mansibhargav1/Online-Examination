...
{ 
	...
	add1()
	{
		...
		try
		{
			...
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/on_exam?useSSL=false","root","renu");
	...
	public void actionPerformed(ActionEvent e)
	{
	 if(e.getSource()==b1)
	 {
		 String s1=t.getText();
			...
			{
			Class.forName("com.mysql.cj.jdbc.Driver");
			Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/on_exam?userSSL=false","root","renu");
			Statement st=con.createStatement();
			if(s1.length()>=1)
			{
	 ...
	 if(e.getSource()==b2)
	 {
		 ...
		 String a3=t3.getText();
			...
			st.executeUpdate("update course set name='"+a2+"',time='"+a3+"' where sno="+a1+"");
